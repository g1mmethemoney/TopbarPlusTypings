...
declare interface Icon {
	 ...
	 */
	convertLabelToNumberSpinner(numberSpinner: NumberSpinner): Icon;
	...
	setXSize(xOffset: number, toggleState?: ToggleState): Icon;

	/** Same as icon:setSize except only for the Y Offset (the X offset is generated automatically). */
	setYSize(yOffset: number, toggleState?: ToggleState): Icon;
	...
	/** Connects to an icon event based upon the given iconEventName
	...
	/**
	 * Calls the function as a separate coroutine with the first argument being the icon itself.
	 * This is useful when needing to extend the behaviour of an icon while remaining in the chain.
	 */
	call(func: (Icon: Icon) => void): Icon;

	/**
	 * Passes the given userdata to the Icons maid to be destroyed/disconnected on the icons
	 * destruction. If a function is passed, it will be executed right away with its self (the icon) being
	 * passed as the first argument. The return value is then given to the maid (instead of the function).
	 */
	give(
	...
	/**
	 * Locks the icon, yields for the given time, then unlocks the icon, effectively shorthand for
	 * icon:lock() task.wait(seconds) icon:unlock(). This is useful for applying cooldowns (to
	 * prevent an icon from being pressed again) after an icon has been selected or deselected.
	 */
	debounce(seconds: number): Icon;

	/**
	 * When set to true (the default) the icon is deselected when another icon (with autoDeselect
	 * enabled) is pressed. Set to false to prevent the icon being deselected when another icon is
	 * selected (a useful behaviour in dropdowns).
	 */
	autoDeselect(toggle: boolean): Icon;
